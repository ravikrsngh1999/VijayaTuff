...
def productdetails(request,type,desc):
    ...
    print(total_area)
    logs = Log.objects.filter(product__glass_type__name = type).filter(product__description=desc)[:5]
    context={
...
def addlog(request,type,desc):
    ...
    if request.method == "POST":
        ...
        p.quantity = str(q)
        p.save()
        Log.objects.create(product=p,initial_quantity=initial_quantity,in_quantity=in_quantity,used_quantity=used_quantity,left_quantity=p.quantity,date=date,user=request.user)
        print("Created")
...
def viewalllogs(request,type,desc):
    logs = Log.objects.filter(product__glass_type__name = type).filter(product__description=desc).order_by("-pk")
    context={
        'logs':logs,
        'type':type,
        'desc':desc,
...
def searchlogs(request):
    ...
    desc = request.GET['desc']
    val = request.GET['val']
    lookups=  Q(product__size__length__icontains=val) | Q(product__size__breadth__icontains=val) | Q(date__icontains=val) | Q(product__size__length__startswith=val) | Q(product__size__breadth__startswith=val) | Q(date__startswith=val)
    p = Log.objects.filter(product__glass_type__name = type).filter(product__description=desc).filter(lookups).order_by("-pk").distinct()
    for i in p:
        l = []
        l.append(i.product.size.__str__())
...
def exportalllogs(request):
    ...
    if user.is_authenticated:
        ...
        for i in logs:
            l = []
            l.append(i.product.glass_type.name)
            l.append(i.product.description)
            l.append(i.product.size.__str__())
            l.append(i.initial_quantity)
            l.append(i.in_quantity)
            l.append(i.used_quantity)
